def chuyen_doi_tien(x, loai_tien):
    ...
    for tien in loai_tien:
        ...
        # Nếu loại tiền này được sử dụng (số tờ > 0), ta cộng vào tổng số tờ và tăng số loại
        if so_to > 0:
            tong_so_to += so_to
            tong_so_loai += 1
...
if a > b:
    ...
    # Trường hợp a < b: Cần thối tiền lại cho khách
    tien_thoi = b - a
    print(f"Số tiền {tien_thoi} được thối lại thành:")
    
    # Gọi hàm tính toán số tờ tiền
    so_to_tien, tong_so_to, tong_so_loai = chuyen_doi_tien(tien_thoi, loai_tien)
    
    # Chỉ in ra màn hình những loại tiền có số tờ > 0
